...
PBYTE256 = ctypes.c_ubyte * 256 # python c tpye lib (c언어에서 지원되는 lib 사용) c_ubyte : unsigned character * 256
_user32 = ctypes.WinDLL("user32") # WinDLL 인스턴스를 만듬 윈도우즈 dll 함수를 로딩 
...
def chat_last_save():
    ...
    # 1. \[ [ 로시작하는것을 찾음
    # 2. () 괄호안의 패턴을 찾음 
    # 3. [] 문자 집합 괄호로 묶인 문자중 하나를 찾음 
    # 4. \S 공백이 아닌 문자를 찾음
    # 5. \s 모든 공백 문자를 찾음 
    # 결국 : [] 로 감싸진 공백이나 공백아닌 문자 패턴을 찾음 

    # 정규식 설명2 - 둘째 음절 
    # 1. \[ [ 로시작하는것을 찾음
    # 2. () 괄호안의 패턴을 찾음 
    # 3. 오전|오후 오전 또는 오후를 찾음 
    # 4. 0-9 모든 숫자 : 
    # 5. \s 모든 공백 문자를 찾음 
    # 결국 오전|오후 11:45 같은 패턴을 찾음 


    # print(df[0])
    # print(f"index -1 은 : {df.index[-1]}, iloc -1은 : { df.iloc[-1, 0]}")
...
def chat_chek_command(cls, clst):
    ...
    if df.iloc[-2, 0] == clst:
        ...
        if 1 <= int(found.count()):
            ...
            realtimeList = "실검없따"  # 네이버 실시간 검색어 상위 20개
